...
describe('data.ts lib unit tests', () => {
    ...
    describe('Events', () => {
        it('getEvents returns mapped array', async () => {
            ...
            (supabase.from('events').select('*').order as jest.Mock).mockResolvedValue({ data: mockData, error: null });
        ...
        it('getEvents returns empty array on error', async () => {
            (supabase.from('events').select('*').order as jest.Mock).mockResolvedValue({ data: null, error: { message: 'DB Error' } });
            const events = await getEvents();
            expect(events).toEqual([]);
        });

        it('getUpcomingEvents filters by today', async () => {
            (supabase.from('events').select('*').gte as jest.Mock).mockReturnThis();
            (supabase.from('events').select('*').gte('date', '').order as jest.Mock).mockResolvedValue({ data: [], error: null });
        ...
        it('getEventById returns undefined on not found', async () => {
            (supabase.from('events').select('*').eq as jest.Mock).mockReturnThis();
            (supabase.from('events').select('*').eq('id', '1').single as jest.Mock).mockResolvedValue({ data: null, error: { message: 'Not found' } });
        ...
        it('saveEvent calls upsert with correct payload', async () => {
            ...
            (supabase.from('events').upsert as jest.Mock).mockResolvedValue({ error: null });
        ...
        it('deleteEvent calls delete with id', async () => {
            (supabase.from('events').delete as jest.Mock).mockReturnThis();
